        ...
        <section className="rounded-[30px] border border-white/10 bg-slate-900/72 p-5 shadow-2xl shadow-slate-950/25 sm:p-6">
          <div className="mb-5">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-400">
              Удаление мягкое: история расходов сохраняет название категории на момент операции.
            </p>
          </div>

          <ActionForm action={createCategory} className="grid gap-4 md:grid-cols-[1fr_auto]" errorClassName={gridErrorClassName}>
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">
            ...
            исторический вариант названия категории.
          </p>
        </div>
          ...
          {activeCategories.map((category) => (
            ...
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <ActionForm action={deleteCategory}>
                  ...
                  <SubmitButton
                    ...
                    confirmMessage={`Удалить категорию "${category.name}"? Старые операции сохранят историческое название категории.`}
              ...
              <details className="mt-5 rounded-[26px] border border-white/8 bg-slate-950/45 p-4">
                ...
                <ActionForm action={updateCategory} className="mt-4 grid gap-4 md:grid-cols-[1fr_auto]" errorClassName={gridErrorClassName}>
                  <input type="hidden" name="categoryId" value={category.id} />
                  <label className="space-y-2 text-sm text-slate-300">
                    <span className="block text-xs uppercase tracking-[0.2em] text-slate-500">
                      Новое имя
              ...
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <ActionForm action={restoreCategory}>
                      <input type="hidden" name="categoryId" value={category.id} />
                      <SubmitButton
                        label={"\u0412\u043e\u0441\u0441\u0442\u0430\u043d\u043e\u0432\u0438\u0442\u044c"}
                        pendingLabel={"\u0412\u043e\u0437\u0432\u0440\u0430\u0449\u0430\u044e..."}
                        ariaLabel={`Восстановить категорию ${category.name}`}
                        confirmMessage={`Восстановить категорию "${category.name}"?`}
                    ...
                    <ActionForm action={hardDeleteCategory}>
                      <input type="hidden" name="categoryId" value={category.id} />
                      <SubmitButton
                        label={"\u0423\u0434\u0430\u043b\u0438\u0442\u044c \u043f\u043e\u043b\u043d\u043e\u0441\u0442\u044c\u044e"}
                        pendingLabel={"\u0423\u0434\u0430\u043b\u044f\u044e..."}
                        disabled={category.expenseCount > 0}
                        ariaLabel={`Полностью удалить категорию ${category.name}`}
                        confirmMessage={`Полностью удалить категорию "${category.name}"? Это действие нельзя отменить.`}
                ...
                {category.expenseCount > 0 ? (
                  <p className="mt-3 text-sm text-slate-500">
                    Полное удаление доступно только для категорий без исторических операций.
                  </p>
                ) : null}
              </article>
            ))}
